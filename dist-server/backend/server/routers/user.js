...
export const userRouter = router({
    getProfile: protectedProcedure
        ...
        if (!user)
            throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to provision user profile' });
    ...
    selectSchool: protectedProcedure
        ...
        if (user.school)
            throw new TRPCError({ code: 'FORBIDDEN', message: 'School selection is irreversible' });
    ...
    getLastBroadcast: protectedProcedure
        ...
        const activeBroadcasts = await db.select()
            ...
            .where(and(eq(globalNotifications.isActive, true), globalNotifications.expiresAt ? gte(globalNotifications.expiresAt, new Date()) : undefined))
