...
import { QrCode, CheckCircle2, ArrowLeft, Upload, Loader2 } from 'lucide-react';

export const YapeCheckout: React.FC = () => {
  const navigate = useNavigate();
  const { modalidad_postulacion } = useUserStore();
  const [isUploading, setIsUploading] = useState(false);
  const [uploadSuccess, setUploadSuccess] = useState(false);

  const submitVoucherMutation = trpc.membership.submitVoucher.useMutation();
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `vouchers/${auth.currentUser.uid}_${Date.now()}.jpg`);
      await uploadBytes(storageRef, compressedBlob);
      const downloadURL = await getDownloadURL(storageRef);

      await submitVoucherMutation.mutateAsync({
        userId: auth.currentUser.uid,
        voucherUrl: downloadURL,
        amount: 15,
        school: modalidad_postulacion || 'NO_ESPECIFICADA',
      });

      setUploadSuccess(true);
      toast.success('Voucher subido correctamente. En breve será validado.');
    } catch (error) {
